...
Model_Path = "./Model/"
Log_path = "./Log/"
Test_path = './Model/GSAEGNN/'
model_time = None
view_model_path = './Dataset/view_model/'
...
def train_one_epoch(model, data_loader):
    ...
    for data in data_loader:
        ...
        _, _, labels, node_features, edge_index, edge_feat, pos, G_batch = data
        if torch.cuda.is_available():
        ...
        y_true = y_true.long()
        y_pred = model(node_features, edge_index, pos, edge_feat, G_batch=G_batch)
...
def evaluate(model, data_loader):
    ...
    for data in data_loader:
        with torch.no_grad():
            sequence_names, _, labels, node_features, edge_index, edge_feat, pos, G_batch = data
            if torch.cuda.is_available():
                node_features = Variable(node_features.cuda().float())
                edge_feat = Variable(edge_feat.cuda().float())
                edge_index = Variable(edge_index.cuda().long())
...
def train(model, train_dataframe, valid_dataframe, fold=0, t=0):
    train_loader = DataLoader(dataset=EGNNProDataset(train_dataframe), batch_size=Config.batch_size, shuffle=True,
                              num_workers=4,
                              collate_fn=egnn_graph_collate,
                              persistent_workers=True, pin_memory=True)
    valid_loader = DataLoader(dataset=EGNNProDataset(valid_dataframe), batch_size=Config.batch_size, shuffle=True,
                              num_workers=4,
                              collate_fn=egnn_graph_collate,
                              persistent_workers=True, pin_memory=True)
    best_epoch = 0
...
def cross_validation(all_dataframe, fold_number=5, layers=10, t=0):
    ...
    for train_index, valid_index in kfold.split(sequence_names, sequence_labels):
        ...
        model = EGNNPPIS(in_dim=67, in_edge_dim=0, hidden_dim=67, layers=layers, attention=True, tanh=False,
                         normalize=False, p=0.6,
                         pool_strategy='GSA')
        if torch.cuda.is_available():
            model.cuda()
        best_epoch, valid_auc, valid_aupr = train(model, train_dataframe, valid_dataframe, fold + 1, t=t)
...
def train_full_model(all_dataframe, test_dataframe, test_psepos_path, p=0.6, pool_strategy="topK", layers=6):
    print("\nTraining a full model using all training data...\n")
    print("p : ", p)
    print("layers: ", layers)
    if Config.is_handcrafted_feature_group:
        model = EGNNPPIS(in_dim=67, in_edge_dim=0, layers=10, hidden_dim=67, attention=True, tanh=False,
                         normalize=False, p=0.6, pool_strategy="GSA")
    else:
        model = EGNNPPIS2(in_dim=3328, in_edge_dim=0, hidden_dim=128, layers=10, attention=True, tanh=False,
                          normalize=False, p=0.6, pool_strategy="GSA")
    if torch.cuda.is_available():
        model.cuda()

    train_loader = DataLoader(dataset=EGNNProDataset(all_dataframe), batch_size=Config.batch_size, shuffle=True,
                              num_workers=4,
                              collate_fn=egnn_graph_collate,
                              persistent_workers=True, pin_memory=True)
    test_loader = DataLoader(dataset=EGNNProDataset(dataframe=test_dataframe, psepos_path=test_psepos_path),
                             batch_size=Config.batch_size,
                             shuffle=True, num_workers=4, collate_fn=egnn_graph_collate,
                             persistent_workers=True, pin_memory=True)
    ans = 0
    best_auprc = -1
    for epoch in range(Config.epochs):
...
def test(test_dataframe, psepos_path):
    print("testing------------------------------")
    test_loader = DataLoader(dataset=EGNNProDataset(dataframe=test_dataframe, psepos_path=psepos_path), batch_size=1,
                             shuffle=True, num_workers=4, collate_fn=egnn_graph_collate)
    print(Test_path)
...
def test_and_save(test_dataframe, psepos_path):
    model_name = ''
    if Config.is_handcrafted_feature_group:
        model = EGNNPPIS(in_dim=67, in_edge_dim=0, layers=10, hidden_dim=67, attention=True, tanh=False, normalize=False, p=0.6, pool_strategy="GSA")
        model_name = 'GSAEGNN.pkl'
    else:
        model = EGNNPPIS2(in_dim=3328, in_edge_dim=0, hidden_dim=128, layers=10, attention=True, tanh=False,
                      normalize=False, p=0.6, pool_strategy="GSA")
        model_name = 'GSAPLM.pkl'
    test_loader = DataLoader(dataset=EGNNProDataset(dataframe=test_dataframe, psepos_path=psepos_path), batch_size=1,
                             shuffle=True, num_workers=4, collate_fn=egnn_graph_collate)
...
def main():
    ...
    # test GSAEgnn
    # test(test_dataframe, pos, layers=10, pool_strategy='GSA')
    # test_and_save(test_dataframe, pos)

    # train GSAEgnn
    train_full_model(train_dataframe, test_dataframe, pos, 0.6, pool_strategy="GSA", layers=10)
